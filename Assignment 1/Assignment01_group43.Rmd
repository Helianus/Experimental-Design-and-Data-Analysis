...
for (i in 1:length(lamdaseq))
  ...
  A[i] = (length(B[B<med])/length(B)) #Building a vector with the values for which B<med, divided by the length of B
}

plot(lamdaseq, c(diff(A), 0), type='l') #for this plot, a random sample has been drawn
#with the lambda values from lambdaseq. For each sample, 1000 observations were drawn and compared to the median sample.
```
...
print(median(Tstar))#The obtained 95% CI for the population median of the given sample is [0.20, 0.32], around its mean 0.26.
```
...
print(ciB)
#The obtained 95% confidence interval for the population median is [21.69263, 32.15699], around the mean 26.46878.
```
...
p = 2*(min(pl,pr))
p
#The bootstrap p-value is 0, which means that the data from the t-test can not not be trusted, hence the hypothesis can not yet be validated or rejected.
...
mean(Tstar)
pl;pr;p
#The obtained p-value from the test is 0.672, hence H0 is validated, confirming there is a significant difference between the samples.
...
boxplot(data_4$before - data_4$after, main="The difference of the two data sets")#Creates a boxplot from the data with the time after the drink
```
