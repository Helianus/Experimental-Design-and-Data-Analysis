...
```{r,echo=FALSE,compress = T, fig.width = 7, fig.height = 4}
par(mfrow=c(1,2))
...
Considering the summary of the data discussed in the previous question, it is clear that the interaction of the 2 factors has a significant effect on the time of the decay.
...
```{r,echo=FALSE,compress = T, fig.width = 5, fig.height = 3}
par(mfrow=c(1,2))
interaction.plot(search_data$skill, search_data$interface, search_data$time, col = c('red', 'blue', 'green'), main = "Skill vs. time" ,xlab="skill" ,ylab="time")
interaction.plot(search_data$interface, search_data$skill, search_data$time, col = c('red', 'blue', 'yellow', 'green', 'black'), main = "Interface vs. time", xlab="interface", ylab="time")
```
...
```{r}
cat("The smallest time is equal with:", mean(search_data$time[search_data$interface == 1]) + mean(search_data$time[search_data$interface == 1]) - mean(search_data$time[search_data$skill == 1]))
```

The interface that has the highest individual and mean search time is number 3, having a standard deviation 4.460 bigger than the normal standard deviation(15.013). The shortest time would be for the first interface(1) and and participant with the highest computational level(1) which is equal with 18.92. If we want to find the time a level 3 user would spend on a level 3 interface we need to compute $mu$ + $alfa_3$ + $Beta_2$ which is equal with 15.013 + 4.460 + 3.033 = 22.506.
...
After computing the Friedman test we obtain a $p$-value of 0.04076, this means that we will reject the null hypothesis H0, so there is a an effect in the interface that is used.
...
The QQ-plot looks normal, and the residuals appear to be evenly distributed so we can conclude that de data has a normal distribution.
...
For a statistical analysis of the word frequencies in relation to Jane Austen's novel Sanditon, the amount of specific words used in her previous works is compared to the ones used by the admirer who tried to replicate her style. For such comparison, it is needed to determine if the frequency counts for specific words are equally distributed for both writers. Therefore, a contingency table test for homogeneity is the most suitable.

#### b)

To analyze Austen's consistency throughout her different novels, a Chi-squared test can be used. This is performed on the columns containing data based on her writings (Sense, Emma, Sand1):
