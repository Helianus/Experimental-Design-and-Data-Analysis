...
After verifying the interaction between the two factors (Sex, Class) and the numerical variable (Age) we obtain a $p$-value of 0.42 which is bigger than 0.05. Because of this we can conclude that we will not reject $H0$ so the numerical variable Age does not have a big influence over the outcome so we can eliminate it from the model.
