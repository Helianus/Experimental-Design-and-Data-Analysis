...
Because the variable Age doesn't have a major influence in the outcome, we can create a model that does not include it.
...
Yes, the second approach is wrong, because it only test one factor at a time - instead of both at the same time. One advantage of this approach is that we can test and see the influence of each factor separately on the Survived status. Based on this approach, we can discard the factors that do not influence the outcome. One disadvantage is that we can only test one variable at at time and we can not see if there is any influence between two factors (ex: between PClass and Sex).
